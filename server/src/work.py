
work = [
    {
        "name": "Stripe",
        "logo": "https://prospercircle-organization-logos.s3.amazonaws.com/production/de9LBGmxDwjvJ36eq.png",
        "position": "Incoming Software Engineering Intern",
        "from": "Jun. 2022",
        "to": "Sept. 2022",
        "background": "Long ways till here...",
        "location": "Seattle, Washington",
        "points": []
    },
    {
        "name": "Robinhood",
        "logo": "https://descargar.org/wp-content/uploads/2017/04/robinhood-iphone-logo.jpg",
        "position": "Incoming Software Engineering Intern",
        "from": "Jan. 2022",
        "to": "May 2022",
        "background": "Once I get there, I will update this!",
        "location": "Menlo Park, California",
        "points": []
    },
    {
        "name": "Amazon",
        "logo": "https://clipartcraft.com/images/amazon-smile-logo-arrow-1.png",
        "position": "Software Engineering Intern",
        "from": "Jun. 2021",
        "to": "Sept. 2021",
        "background": "I worked in a backend team in Prime Video Ads. My project involved building a scalable stream processing service to aggregate ad reporting data across multiple sources.",
        "location": "Seattle, Washington",
        "points": [
            "Building a real-time service to aggregate high-throughput ad reports and logs using Java, AWS Kinesis, Apache Flink, and Elasticsearch",
            "Developed a bulk Elasticsearch writer using AWS Lambda and Dagger, batching up to 1000 output data-points in one call",
            "Implemented a feature to filter duplicate ad logs in Flink, reducing Flink and Elasticsearch memory requirements by around 50x",
            "Utilizing Kibana to visualize the aggregated data, reducing diagnosis time of ad reporting issues from weeks to seconds"
        ]
    },
    {
        "name": "MathWorks",
        "logo": "https://www.mathworks.com/help/examples/matlab/win64/MatlabLogoExample_08.png",
        "position": "Software Engineering Intern",
        "from": "Jan. 2021",
        "to": "May 2021",
        "background": "I worked on JavaScript infrastructure in the Web/UI Widgets team, a team responsible for creating a UI framework used by most of MathWorks; My primary task was to drive widget development from AMD to ECMAScript.",
        "location": "Boston, Massachusets",
        "points": [
            "Created an internal tool with Jscodeshift and Node.js to convert AMD source code into ECMAScript, enabling development of UI widgets in ECMAScript",
            "Extended the project generator tool to scaffold a new ECMAScript widget in seconds with Yeoman",
            "Added import-map generation to existing widget infrastructure to enable testing ECMAScript widgets in the deployment pipeline"]
    },
    {
        "name": "TigerGraph",
        "logo": "https://www.ambient-it.net/wp-content/uploads/2021/07/logo-formation-tigergraph.png",
        "position": "Solutions Architect Intern",
        "from": "Jul. 2020",
        "to": "Sept. 2020",
        "background": "TigerGraph is a graph database as a service company. My role involved enhancing TigerGraph's algorithm library and build customers' dream graph solutions.",
        "points": [
            "Added the KCore algorithm to TigerGraph’s algorithm library and nearly 12 new unit tests for each algorithm",
            "Utilized Python to generate random graphs, automating the creation of unit test data for the algorithms",
            "Advised and implemented schema and query performance improvements for clients’ graph database solutions",
        ],
        "location": "Redwood City, California"
    },
    {
        "name": "Futurist Academy",
        "logo": "data:image/png;base64,",
        "position": "Software Engineering Intern",
        "from": "Aug. 2019",
        "to": "Jul. 2020",
        "background": "I worked on a platform called OrgX which essentially connects users, projects, events and organizations.",
        "points": [
            "Helped develop the core OrgX back-end services with Node.js, Express, Firebase Cloud Functions and TigerGraph",
            "Designed and developed a group chat feature using Cloud Firestore, and built React components to integrate it into OrgX’s web client",
            "Created both activity feed and user notification features using the Stream API, Cloud Firestore, and TigerGraph, serving real-time updates to OrgX’s web client",
        ],
        "location": "Minneapolis, MN"
    }]
